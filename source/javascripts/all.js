...
function copyEmailToClipboard() {
  const emailText = 'magnus.gyllensward@gmail.com';
  const button = document.querySelector('.copy-email-btn');
  const emailSpan = button.querySelector('.email-text');
  const feedbackSpan = button.querySelector('.copy-feedback');

  // Use modern clipboard API if available
  if (navigator.clipboard) {
    navigator.clipboard.writeText(emailText).then(() => {
      showCopyFeedback(emailSpan, feedbackSpan);
    }).catch((err) => {
      console.error('Failed to copy email: ', err);
      fallbackCopyTextToClipboard(emailText, emailSpan, feedbackSpan);
    });
  } else {
...
function fallbackCopyTextToClipboard(text, emailSpan, feedbackSpan) {
...
function showCopyFeedback(emailSpan, feedbackSpan) {
